        ...
        <div class="card-body">
            <form method="POST" action="{{ route("admin.employers.update", [$employer->id]) }}"
                ...
                <div class="form-group">
                    ...
                    <select class="form-control select2 {{ $errors->has('countries') ? 'is-invalid' : '' }}"
                            name="countries[]" id="countries" multiple>
                        @foreach($countries as $id => $countries)
                            <option
                                value="{{ $id }}" {{ (in_array($id, old('countries', [])) || $employer->countries->contains($id)) ? 'selected' : '' }}>{{ $countries }}</option>
                        @endforeach
                    </select>
