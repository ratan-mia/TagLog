    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.visas.store") }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('countries_without_visa') ? 'is-invalid' : '' }}" name="countries_without_visa" id="countries_without_visa">
                    ...
                    @foreach(App\Visa::COUNTRIES_WITHOUT_VISA_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('countries_without_visa', '') === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                @foreach(App\Visa::WORK_PERMIT_RADIO as $key => $label)
                    <div class="form-check {{ $errors->has('work_permit') ? 'is-invalid' : '' }}">
                        <input class="form-check-input" type="radio" id="work_permit_{{ $key }}" name="work_permit" value="{{ $key }}" {{ old('work_permit', '') === (string) $key ? 'checked' : '' }}>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('working_limit') ? 'is-invalid' : '' }}" type="number" name="working_limit" id="working_limit" value="{{ old('working_limit') }}" step="1">
            ...
            <div class="form-group">
                ...
                @foreach(App\Visa::LANGUAGE_TRANING_RADIO as $key => $label)
                    <div class="form-check {{ $errors->has('language_traning') ? 'is-invalid' : '' }}">
                        <input class="form-check-input" type="radio" id="language_traning_{{ $key }}" name="language_traning" value="{{ $key }}" {{ old('language_traning', '') === (string) $key ? 'checked' : '' }}>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('training_duration') ? 'is-invalid' : '' }}" type="number" name="training_duration" id="training_duration" value="{{ old('training_duration') }}" step="1">
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('countries_restrictred') ? 'is-invalid' : '' }}" name="countries_restrictred" id="countries_restrictred">
                    ...
                    @foreach(App\Visa::COUNTRIES_RESTRICTRED_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('countries_restrictred', '') === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
