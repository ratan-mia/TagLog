    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.users.update", [$user->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('destination_country') ? 'is-invalid' : '' }}" name="destination_country_id" id="destination_country_id">
                    @foreach($destination_countries as $id => $destination_country)
                        <option value="{{ $id }}" {{ ($user->destination_country ? $user->destination_country->id : old('destination_country_id')) == $id ? 'selected' : '' }}>{{ $destination_country }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('expected_industries') ? 'is-invalid' : '' }}" name="expected_industries[]" id="expected_industries" multiple>
                    @foreach($expected_industries as $id => $expected_industries)
                        <option value="{{ $id }}" {{ (in_array($id, old('expected_industries', [])) || $user->expected_industries->contains($id)) ? 'selected' : '' }}>{{ $expected_industries }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('expected_salary') ? 'is-invalid' : '' }}" type="number" name="expected_salary" id="expected_salary" value="{{ old('expected_salary', $user->expected_salary) }}" step="0.01">
            ...
            <div class="form-group">
                ...
                <input class="form-control date {{ $errors->has('date_of_leaving') ? 'is-invalid' : '' }}" type="text" name="date_of_leaving" id="date_of_leaving" value="{{ old('date_of_leaving', $user->date_of_leaving) }}">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('employer') ? 'is-invalid' : '' }}" name="employer_id" id="employer_id">
                    @foreach($employers as $id => $employer)
                        <option value="{{ $id }}" {{ ($user->employer ? $user->employer->id : old('employer_id')) == $id ? 'selected' : '' }}>{{ $employer }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('agents') ? 'is-invalid' : '' }}" name="agents_id" id="agents_id">
                    @foreach($agents as $id => $agents)
                        <option value="{{ $id }}" {{ ($user->agents ? $user->agents->id : old('agents_id')) == $id ? 'selected' : '' }}>{{ $agents }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('indurstry') ? 'is-invalid' : '' }}" name="indurstry_id" id="indurstry_id">
                    @foreach($indurstries as $id => $indurstry)
                        <option value="{{ $id }}" {{ ($user->indurstry ? $user->indurstry->id : old('indurstry_id')) == $id ? 'selected' : '' }}>{{ $indurstry }}</option>
                    @endforeach
                </select>
